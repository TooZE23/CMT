...
def cal_flops(
    ...
    if isinstance(modals, dict):
        modal_names = list(modals.keys())
    else:
        modal_names = list(modals)

    def infer_c(name: str) -> int:
        if channels_by_modal and name in channels_by_modal:
            return int(channels_by_modal[name])
        lname = name.lower()
        if "thermal" in lname or lname in {"ir", "infrared"}:
            return 3
        return 3
    ...
    for name in modal_names:
        C = infer_c(name)
        x[name] = torch.zeros(1, C, H, W, dtype=torch.float32)
